def run(shutit_sessions, machines):
	...
	shutit_session.send('apt install -y docker.io')
	shutit_session.send('docker login -p dockeristhelord -u imiell')
	shutit_session.send('git clone https://github.com/nolar/ephemeral-volume-claims')
	shutit_session.send('cd ephemeral-volume-claims')
	shutit_session.send('''cat > Dockerfile << EOF
FROM python:3.7
RUN pip install kopf && pip install kubernetes
COPY ephemeral.py /ephemeral.py
CMD kopf run --standalone /ephemeral.py
EOF''')
	shutit_session.send('docker image build -t imiell/ephemeral-volume-claims:latest .')
	shutit_session.send('docker image push imiell/ephemeral-volume-claims:latest')
	shutit_session.pause_point('continue via https://kopf.readthedocs.io/en/stable/walkthrough/problem/')
