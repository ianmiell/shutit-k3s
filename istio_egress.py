def run(shutit_sessions, machines):
	...
	shutit_session.send('kubectl exec "$SOURCE_POD" -c sleep -- curl -sSI https://www.google.com 2>&1', exit_values=['35'], note='Check that we cannot go out to google')
...
EOF
	...
	shutit_session.send('kubectl exec "$SOURCE_POD" -c sleep -- curl -sSI https://www.bbc.co.uk 2>&1', exit_values=['35'], note='But we still cannot go to the bbc')
...
EOF''', note='Set up VirtualService to direct traffic from the sidecars to the egress gateway to the external service')
	...
	# Create a second sleep in a separate namespace:
	shutit_session.send('kubectl create ns istio-test')
	shutit_session.send('kubectl label namespace istio-test istio-injection=enabled')
	shutit_session.send('sleep 20')
	shutit_session.send('kubectl apply -n istio-test -f https://raw.githubusercontent.com/istio/istio/release-1.12/samples/sleep/sleep.yaml')
	shutit_session.send('export SOURCE_POD=$(kubectl get -n istio-test pod -l app=sleep -o jsonpath={.items..metadata.name})')
	shutit_session.send('kubectl exec "$SOURCE_POD" -c sleep -- curl -sSI https://www.google.com 2>&1', exit_values=['35'], note='Check that we cannot go out to google on the parallel pod')
	shutit_session.pause_point('EGRESS DONE')
